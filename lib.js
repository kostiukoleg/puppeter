...
function getPriceItem () {
    let dollar = 24.27 + (24.27 * 0.3),
        item = {},
        arr = [];
    for (let textData in wb.Sheets['Sheet1']) {
        let num = textData.slice(1, textData.length);
        if (textData.slice(0, 1) === "C") {
            if (wb.Sheets['Sheet1'][textData]['v'] === 0) {
                textData = textData.replace(/C/gi, "B");
                item['category'] = wb.Sheets['Sheet1'][textData]['v'];
            } else {
                let v = textData.replace(/C/gi, "B");
                item['name'] = wb.Sheets['Sheet1'][v]['v'];
                item['price'] = (wb.Sheets['Sheet1'][textData]['v'] * dollar).toFixed(2);
...
async function getScrabProducts (link) {
 ...
 async function getScrabNsProducts (link) {
    try {
        ...
        let data = {
            'cat': 'Межкомнатные двери/Новый стиль/' +category[1],
            'url_cat': 'interior-doors/new-style/'+rusToLatin(category[1].replace( /[“”]/g, "" )),
            'goods': "Межкомнатные двери Новый Стиль "+title.replace( /[“”]/g, "" ),
            'variant': null,
